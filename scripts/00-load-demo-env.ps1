...
.EXAMPLE
    ...
    $DemoEnv.avd.hostPoolName          # hp-avd-mms26-demo-eus-01
    $DemoEnv.resourceGroups.avd        # rg-avd-mms26-demo-eus-01
    ...
    .\scripts\01-prepare-azure-local.ps1 `
        ...
        -ResourceGroupName $DemoEnv.resourceGroups.avd `
        -ClusterName $DemoEnv.cluster.name `
...
# Validate: fail if any TODO placeholders remain
...
Write-Host "     AVD RG       : $($DemoEnv.resourceGroups.avd)"
Write-Host "     Cluster RG   : $($DemoEnv.resourceGroups.cluster)"
Write-Host "     Cluster Name : $($DemoEnv.cluster.name)"
